...
async def jugar_ataque(match_id: int = Form(), card_id: int = Form(),
    ...
    try:
        ...
        # await juego.mensaje_personal(player_objective.id, "H")
...
async def jugar_defensa(match_id: int = Form(), card_id: int = Form(),
    ...
    try:
        juego = get_global_juego(match_id)
        if card_id!=0:
            validar_carta(card_id, player_objective, juego)
        ...
        if card_id == 0:
            ...
            # jugador_ataque.descartar_carta(carta_ataque)
            juego.responder_ataque(player_objective, card_id)
            # await juego.broadcast_global("C")
        else:
            ...
            await juego.broadcast_global({"carta_id": card_id,
                                          "mensaje": msg["mensaje"]})
            # jugador_ataque.descartar_carta(carta_ataque)
            # jugador_defensa.descartar_carta(carta_defensa)
            juego.responder_ataque(player_objective, card_id)
