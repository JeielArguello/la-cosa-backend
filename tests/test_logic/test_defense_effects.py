...
@pytest.mark.asyncio
async def test_defensa_fallaste_success(mocker,mock_juego_async, mock_atacante, mock_objetivo, mock_card_id, mock_proximo, mock_proximo2):
    mock_intercambio = IntercambiarCarta(mock_objetivo, mock_card_id, mock_atacante)
    mock_juego_async.solicitud_intercambio = mock_intercambio
    mock_juego_async.jugadores_en_partida = [mock_atacante, mock_objetivo, mock_proximo, mock_proximo2]
    mock_card_id = 78
    mock_objetivo.cartas = [5, 6, 78, 8]
    mocker.patch("logic.defense_effects.Juego.mensaje_personal", return_value=None)
...
@pytest.mark.asyncio
async def test_defensa_fallaste_fail(mock_juego_async, mock_atacante, mock_objetivo, mock_card_id, mock_proximo, mock_proximo2):
    mock_intercambio = None
    mock_juego_async.solicitud_intercambio = mock_intercambio
    mock_juego_async.jugadores_en_partida = [mock_atacante, mock_objetivo, mock_proximo, mock_proximo2]
